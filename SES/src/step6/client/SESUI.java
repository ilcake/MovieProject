...
public class SESUI extends JFrame implements ActionListener {
	...
	public SESUI() {
		...
		lbl_title = new JLabel("Soft Engineer School Manager", JLabel.CENTER);
		...
		sp_center.setBorder(
				new TitledBorder(null, "\uB4F1\uB85D\uC815\uBCF4", TitledBorder.CENTER, TitledBorder.TOP, null, null));
		...
		cb_humanSelect.setModel(new DefaultComboBoxModel(new String[] { "\uB300\uC0C1\uC120\uD0DD", "\uAD50\uC218",
				"\uC5F0\uC218\uC0DD", "\uC6B4\uC601\uC9C4" }));
		...
		manager = new SESClientManager(this);
		thc = new Thread((Thread) manager);
		initButton(true);
		initField("All", false);

		thc.start();
		manager.getHumanList();
		...
		if (source == btn_insert) {
			...
			if (selectedValue == null) {
				...
				if (selectedValue instanceof Professor)
					selectedHuman = "교수";
				if (selectedValue instanceof Trainee)
					selectedHuman = "연수생";
				if (selectedValue instanceof Staff)
					selectedHuman = "운영진";
