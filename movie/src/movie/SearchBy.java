...
public class SearchBy { 	//감독으로 검색(1) 또는 제목(2)으로 검색 
	...
	public SearchBy(int type, String name) throws Exception{
	...
	public static void main(String[] args) {
		try {
//			new SearchByDirector(1, "이준익");//여기에 검색 값 넣기! 제일 먼저 해야함
			new SearchBy(2, "신비한");//1:감독이름으로 검색, 2 : 제목으로 검색
		} catch (Exception e) {
	...
	public String jsonParsingforDaum(String pra, String title) throws Exception {	//다음에서 섬네일과 줄거리
		String result = "";
		...
		for (int i = 0; i < array.size(); i++) {
			...
			JSONArray storyArray = (JSONArray) entity.get(pra);//story 또는 thumbnail
	...
	public ArrayList<String> jsonParsingforKodis(String url, String name) throws Exception  {	//영진회에서 감독으로 코드
		String result = "";
	...
	public String jsonParsingforKodis2(String pra, String code) throws Exception {	//영진회에서 코드로 정보들
		String result="";
	 ...
	 private static String readKobisUrl(String addUrl, String  name) throws Exception {	//영진회에서 감독 또는 제목 으로 코드
	...
	private static String readDaumUrl(String q) throws Exception {	//다음에서 제목으로
	 ...
	 private static String readInfoUrl(String code) throws Exception {	//영진회에서 코드로 검색
	        BufferedInputStream reader1 = null;
	        try {
	            URL url = new URL(
